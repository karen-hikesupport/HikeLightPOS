...
namespace HikePOS.Helpers
{
    public class CommonMethods
    {
        ...
        public static StringBuilder DocketReceiptStringBuilder(InvoiceDto invoice, ReceiptTemplateDto receiptTemplate, OutletDto_POS CurrentOutlet, int Imin_LINE_WIDTH, int Imin_Column_Width, bool isutf = false)
        {
            StringBuilder sb = new StringBuilder();
            if (receiptTemplate != null)
            {
                ...
                sb.Append(CenterText(NormalText(invoice.FinalizeDateStoreDate.ToString("hh.mmtt, dd MMM yyyy"), isutf), Imin_LINE_WIDTH, 2, isutf));
                sb.Append(CenterText(NormalText("Customer # " + Settings.PrintCustomerCurrentNumber, isutf), Imin_LINE_WIDTH, 2, isutf));
                if (invoice.CustomerId.HasValue)
                {
                    sb.Append(CenterText(NormalText(invoice.CustomerName, isutf), Imin_LINE_WIDTH, 2, isutf));
                }

                sb.Append("\n");
                sb.Append(CenterText(NormalText("Served by " + invoice.ServedByName + ',' + invoice.RegisterName, isutf), Imin_LINE_WIDTH, 2, isutf));

                sb.Append("\n");
                var dsiplayline = new string('-', Imin_LINE_WIDTH);

                int cnt = 0;
                var items = invoice.InvoiceLineItems.Where(x => x.InvoiceItemType != Enums.InvoiceItemType.Discount).ToList();
                foreach (var item in items)
                {
                    if (!item.IsExtraproduct)
                        sb.Append(NormalText(dsiplayline, isutf) + "\n");

                    var clmn1 = new string(' ', 1) + item.Quantity.ToString();
                    if (item.IsExtraproduct)
                    {
                        clmn1 = new string(' ', 1) + clmn1.Substring(1);
                    }
                    var clmn2 = new string(' ', 1) + "@" + new string(' ', 1);
                    var clmn3 = item.ProductTitle;
                    var columnsize = clmn1.Length + clmn2.Length;
                    int nameWidth = Imin_LINE_WIDTH - columnsize;
                    string str = string.Empty;

                    bool priceonfirstline = false;
                    while (clmn3.Length > nameWidth)
                    {
                        if (!priceonfirstline)
                        {
                            str += NormalText(clmn1 + clmn2 + clmn3.Substring(0, nameWidth), isutf) + "\n";
                            priceonfirstline = true;
                        }
                        else
                        {
                            str += NormalText(new string(' ', (nameWidth - clmn3.Length) + columnsize) + clmn3.Substring(0, nameWidth), isutf) + "\n";
                        }
                        clmn3 = clmn3.Substring(nameWidth);
                    }

                    if (clmn3.Length <= nameWidth)
                    {
                        if (priceonfirstline)
                        {
                            str += NormalText(new string(' ', columnsize) + clmn3 + new string(' ', (nameWidth - clmn3.Length)), isutf);
                        }
                        else
                        {
                            str += NormalText(clmn1 + clmn2 + clmn3 + new string(' ', nameWidth - clmn3.Length), isutf);
                        }
                    }
        ...
        public static StringBuilder EpsonDocketReceiptStringBuilder(InvoiceDto invoice, ReceiptTemplateDto receiptTemplate, OutletDto_POS CurrentOutlet, int Imin_LINE_WIDTH, int Imin_Column_Width, bool isutf = true)
        {
            StringBuilder sb = new StringBuilder();
            if (receiptTemplate != null)
            {
                if (!string.IsNullOrEmpty(invoice.Number) || !string.IsNullOrEmpty(receiptTemplate.InvoiceNoPrefix))
                {
                    sb.Append(CommonMethods.NormalText(receiptTemplate.InvoiceNoPrefix + " " + invoice.Number, isutf) + "\n");
                }

                sb.Append(CommonMethods.NormalText(invoice.FinalizeDateStoreDate.ToString("hh.mmtt, dd MMM yyyy"), isutf) + "\n");
                sb.Append(CommonMethods.NormalText("Customer # " + Settings.PrintCustomerCurrentNumber, isutf) + "\n");
                if (invoice.CustomerId.HasValue)
                {
                    sb.Append(CommonMethods.NormalText(invoice.CustomerName, isutf) + "\n");
                }

                sb.Append("\n");
                sb.Append(CommonMethods.NormalText("Served by " + invoice.ServedByName + ',' + invoice.RegisterName, isutf) + "\n");
                ...
                {
                    if (!item.IsExtraproduct)
                        sb.Append(CommonMethods.NormalText(dsiplayline, isutf) + "\n");
                    ...
                    while (clmn3.Length > nameWidth)
                    {
                        if (!priceonfirstline)
                        {
                            str += CommonMethods.NormalText(clmn1 + clmn2 + clmn3.Substring(0, nameWidth), isutf) + "\n";
                            priceonfirstline = true;
                        }
                        else
                        {
                            str += CommonMethods.NormalText(new string(' ', (nameWidth - clmn3.Length) + columnsize) + clmn3.Substring(0, nameWidth), isutf) + "\n";
                        }
                        clmn3 = clmn3.Substring(nameWidth);
                    }
                    ...
                    if (clmn3.Length <= nameWidth)
                    {
                        if (priceonfirstline)
                        {
                            str += CommonMethods.NormalText(new string(' ', columnsize) + clmn3 + new string(' ', (nameWidth - clmn3.Length)), isutf);
                        }
                        else
                        {
                            str += CommonMethods.NormalText(clmn1 + clmn2 + clmn3 + new string(' ', nameWidth - clmn3.Length), isutf);
                        }
                    }
        ...
        public static StringBuilder VantiveStringBuilder(VantivReceiptPrintModel VantivReceiptInfo, InvoiceDto invoice, ReceiptTemplateDto receiptTemplate, OutletDto_POS CurrentOutlet, int Imin_LINE_WIDTH, int Imin_Column_Width, bool isutf = false)
        {
            StringBuilder sb = new StringBuilder();
            if (receiptTemplate != null)
            {
                ...
                sb.Append("\n");
                sb.Append(CenterText(NormalText(VantivReceiptInfo.TransactionDateTime.ToString(), isutf), Imin_LINE_WIDTH, 2, isutf));
                sb.Append("\n");

                if (!string.IsNullOrEmpty(VantivReceiptInfo.ApplicationLabel))
                {
                    sb.Append(CenterText(NormalText(VantivReceiptInfo.ApplicationLabel, isutf), Imin_LINE_WIDTH, 2, isutf));
                }
                if (!string.IsNullOrEmpty(VantivReceiptInfo.ApplicationIdentifier))
                {
                    sb.Append(CenterText(NormalText(VantivReceiptInfo.ApplicationIdentifier, isutf), Imin_LINE_WIDTH, 2, isutf));
                }
                sb.Append("\n");
                sb.Append(NormalText("Merchant ID: " + VantivReceiptInfo.MerchantID, isutf) + "\n");
                sb.Append(NormalText("Terminal ID: " + VantivReceiptInfo.TerminalID, isutf) + "\n");
                sb.Append("\n");
                sb.Append(NormalText("Reference # " + VantivReceiptInfo.VantivReference, isutf) + "\n");
                sb.Append(NormalText("Account # " + VantivReceiptInfo.AccountNo, isutf) + "\n");
                sb.Append(NormalText("Logo: " + VantivReceiptInfo.CardLogo, isutf) + "\n");
                sb.Append(NormalText("Entry: " + VantivReceiptInfo.CardEntry, isutf) + "\n");
                sb.Append("\n");
                sb.Append(NormalText("Transaction ID: " + VantivReceiptInfo.TransactionID, isutf) + "\n");
                if (VantivReceiptInfo.TransactionStatus == "Approved")
                {
                    sb.Append(NormalText("Approval Code: " + VantivReceiptInfo.ApprovalCode, isutf) + "\n");
                }
                sb.Append(NormalText("Response Code: " + VantivReceiptInfo.ResponseCode, isutf) + "\n");
                sb.Append("\n");
                sb.Append(NormalText(VantivReceiptInfo.TransactionStatus, isutf) + "\n");
                sb.Append("\n");
                if (!string.IsNullOrEmpty(VantivReceiptInfo.Cryptogram))
                {
                    sb.Append(NormalText(VantivReceiptInfo.Cryptogram, isutf) + "\n");
                    sb.Append("\n");
                }
                sb.Append(NormalText("Sale: " + VantivReceiptInfo.VantivSale.ToString("c"), isutf) + "\n");
                if (VantivReceiptInfo.PaymentType == "Debit")
                {
                    sb.Append(NormalText("Cashback: " + VantivReceiptInfo.VantivCashBack.ToString("c"), isutf) + "\n");
                    sb.Append(NormalText("Total: " + VantivReceiptInfo.VantivTotal.ToString("c"), isutf) + "\n");
                }
                if (VantivReceiptInfo.WasPinVerified)
                {
                    sb.Append("\n");
                    sb.Append(NormalText("PIN VERIFIED", isutf) + "\n");
                }

                var dsiplayline = new string('-', Imin_LINE_WIDTH);

                if (VantivReceiptInfo.WasPinVerified && VantivReceiptInfo.TransactionStatus == "Approved")
                {
                    sb.Append("\n");
                    sb.Append(NormalText("x", isutf) + "\n");
                    sb.Append(NormalText(dsiplayline, isutf) + "\n");
                }
                if (VantivReceiptInfo.TransactionStatus == "Approved")
                {
                    sb.Append(NormalText("A agree to pay the above total amount according to the card issuer agreement", isutf) + "\n");
                }
                if (VantivReceiptInfo.TransactionStatus == "Declined")
                {
                    foreach (var item in VantivReceiptInfo.EmvTags)
                    {
                        sb.Append(NormalText(item, isutf) + "\n");
                    }
                }
                sb.Append("\n");
                sb.Append(NormalText("Thank You", isutf) + "\n");

            }
            return sb;
        }
        public static StringBuilder CashRegisterStringBuilder(RegisterclosureDto Registerclosure, int Imin_LINE_WIDTH, bool isutf = false)
        {
            StringBuilder sb = new StringBuilder();

            var dsiplayline = new string('-', Imin_LINE_WIDTH);
            int secondcolumlenght = 16;
            ...
            if (Registerclosure.StartDateTime != null)
            {
                if (Registerclosure.EndDateTime == null)
                    date = string.Format(" Opened at {0:dd MMM, yyyy hh.mmtt}", Registerclosure.StartDateTime.Value.ToStoreTime());
                else
                    date = string.Format(" Opened at {0:hh.mmtt} on {0:dddd, dd MMMM yyyy} and Closed at {1:hh.mmtt} on {1:dddd, dd MMMM yyyy}", Registerclosure.StartDateTime.Value.ToStoreTime(), Registerclosure.EndDateTime.Value.ToStoreTime());
            }
            sb.Append(CenterText(NormalText(Registerclosure.RegisterName, isutf), Imin_LINE_WIDTH, 2, true));
            sb.Append("\n");
            sb.Append(CenterText(NormalText(string.IsNullOrEmpty(date) ? "  " : date, isutf), Imin_LINE_WIDTH, 2, true));
            sb.Append("\n");
            sb.Append(CenterText(NormalText(Registerclosure.OutletName, isutf), Imin_LINE_WIDTH, 2, true));
            sb.Append(NormalText(dsiplayline, isutf) + "\n");


            sb.Append(NormalText(WrapItemLine("Total sales:", (Registerclosure.TotalSales ?? 0).ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            sb.Append(NormalText(WrapItemLine("Tax collected:", (Registerclosure.TotalTax ?? 0).ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            foreach (var tax in Registerclosure.TaxList)
            {
                sb.Append(NormalText(WrapItemLine(tax.TaxFullName, tax.TaxAmount.ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
                sb.Append("\n");
            }
            sb.Append(NormalText(WrapItemLine("Discounts:", (Registerclosure.Discounts ?? 0).ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            sb.Append(NormalText(WrapItemLine("Tip/surcharge:", (Registerclosure.TotalTip ?? 0).ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            sb.Append(NormalText(WrapItemLine("Shipping:", (Registerclosure.TotalShipping ?? 0).ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            sb.Append(NormalText(WrapItemLine("Payments:", (Registerclosure.TotalPayments ?? 0).ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            sb.Append(NormalText(WrapItemLine("Net receipts:", Registerclosure.NetReceipts.ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append(NormalText(dsiplayline, isutf) + "\n");

            if (!string.IsNullOrEmpty(Registerclosure.Notes))
            {
                sb.Append(NormalText("Note: " + Registerclosure.Notes + "\n", isutf));
                sb.Append(NormalText(dsiplayline, isutf) + "\n");
            }
            ...
            if (registerclosuresTallyDto == null)
                registerclosuresTallyDto = new RegisterclosuresTallyDto();
            decimal TotalCashSale = 0;
            if (Registerclosure.RegisterCashInOuts != null)
                TotalCashSale = registerclosuresTallyDto.ActualTotal - Registerclosure.RegisterCashInOuts.Sum(x => x.Amount);
            sb.Append(NormalText(WrapItemLine("Cash in till expected:", registerclosuresTallyDto.ActualTotal.ToString("C"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            sb.Append(NormalText(WrapItemLine("Cash in till actual:", registerclosuresTallyDto.RegisteredTotal.ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            sb.Append(NormalText(WrapItemLine("Cash sale:", TotalCashSale.ToString("c"), Imin_LINE_WIDTH, 14, isutf), isutf));
            sb.Append("\n");
            sb.Append(NormalText(WrapItemLine("Cash difference:", registerclosuresTallyDto.DifferenceTotal.ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append(NormalText(dsiplayline, isutf) + "\n");

            sb.Append(BoldText(WrapItemLine("Payment type (Expected)", "Counted", Imin_LINE_WIDTH, 9, isutf), isutf));
            sb.Append("\n");
            foreach (var item in Registerclosure.RegisterclosuresTallys)
            {
                sb.Append(NormalText(WrapItemLine(item.PaymentOptionName + " (Expected:-" + item.ActualTotal.ToString("c") + ")", item.RegisteredTotal.ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
                sb.Append("\n");
            }

            sb.Append(NormalText(dsiplayline, isutf) + "\n");
            sb.Append(BoldText(WrapItemLine("Transaction/User", "Amounts", Imin_LINE_WIDTH, 9, isutf), isutf));
            sb.Append("\n");
            foreach (var item in Registerclosure.RegisterCashInOuts)
            {
                sb.Append(NormalText(WrapItemLine(item.RegisterCashTypeName + "(User:-" + item.UserBy + ")", item.Amount.ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
                if (!string.IsNullOrEmpty(item.Note))
                {
                    sb.Append(NormalText(WrapItemLine("Note: " + item.Note, " ", Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
                }
                sb.Append("\n");
            }

            sb.Append(NormalText(dsiplayline, isutf) + "\n");
            sb.Append(BoldText(WrapItemLine("Total cash in:", Registerclosure.RegisterTotalCashIn.ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            sb.Append(BoldText(WrapItemLine("Total cash out:", Registerclosure.RegisterTotalCashOuts.ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append(NormalText(dsiplayline, isutf) + "\n");

            sb.Append(NormalText(WrapItemLine(LanguageExtension.Localize("NewCustomersText"), Registerclosure.registerClosureTransactionDetailsDto.TotalNewCustomers.ToString(), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            sb.Append(NormalText(WrapItemLine(LanguageExtension.Localize("NumberOfTransactionsText"), Registerclosure.registerClosureTransactionDetailsDto.TotalTransactions.ToString(), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append("\n");
            sb.Append(NormalText(WrapItemLine(LanguageExtension.Localize("AverageSaleValueText"), Registerclosure.registerClosureTransactionDetailsDto.AvgSaleAmount.ToString("c"), Imin_LINE_WIDTH, secondcolumlenght, isutf), isutf));
            sb.Append(NormalText(dsiplayline, isutf) + "\n");

            sb.Append(NormalText("Opened by: " + Registerclosure.StartByUser, isutf));
            if (Settings.CurrentRegister?.LastRegisterclosure?.CloseByUser != null && !Settings.CurrentRegister.IsOpened)
            {
                sb.Append("\n");
                sb.Append(NormalText("Closed by: " + Settings.CurrentRegister.LastRegisterclosure.CloseByUser, isutf));
            }
            sb.Append("\n");
            return sb;
        }
